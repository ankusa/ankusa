...
function scarlet_witch($VISION, $JARVIS, $LEO)
{
    ...
    $LEO = implode(':', $LEO);
    $PBKDF2_ITERATIONS = 10000;  // Reduced from 100000

    if ($VISION == "encrypt") {
        $salt = openssl_random_pseudo_bytes(16);  
        $IV = openssl_random_pseudo_bytes(12); 
        $start = microtime(true);
        $DOCTOR_STRANGE = hash_pbkdf2('sha256', $LEO, $salt, $PBKDF2_ITERATIONS, 32, true); 
        $tag = null;
        $encrypted = openssl_encrypt($JARVIS, $METHOD, $DOCTOR_STRANGE, OPENSSL_RAW_DATA, $IV, $tag);
        $duration = microtime(true) - $start;
        file_put_contents("decrypt.log", "[ENCRYPT] PBKDF2 took: {$duration}s
", FILE_APPEND);
    ...
    elseif ($VISION == "decrypt") {
        ...
        $duration = microtime(true) - $start;
        file_put_contents("decrypt.log", "[DECRYPT] PBKDF2 took: {$duration}s
", FILE_APPEND);
